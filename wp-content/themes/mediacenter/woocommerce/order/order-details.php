...
<section class="section order-details">
	<h2><?php _e( 'Order Details', 'media_center' ); ?></h2>
	<table class="shop_table order_details table">
		<thead>
			<tr>
				<th class="product-name"><?php _e( 'Product', 'media_center' ); ?></th>
				<th class="product-total"><?php _e( 'Total', 'media_center' ); ?></th>
			</tr>
		</thead>
		<tbody>
			<?php
								...
								<?php
									...
									if ( $_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted() ) {
										...
										foreach ( $download_files as $download_id => $file ) {
											$i++;
											$links[] = '<small><a href="' . esc_url( $file['download_url'] ) . '">' . sprintf( __( 'Download file%s', 'media_center' ), ( count( $download_files ) > 1 ? ' ' . $i . ': ' : ': ' ) ) . esc_html( $file['name'] ) . '</a></small>';
										}
		...
		<?php
					...
					if ( $has_refund && $key === 'order_total' ) {
						...
						if ( wc_tax_enabled() && 'incl' == $order->tax_display_cart ) {
							...
							if ( ! empty( $tax_del_array ) ) {
								$refunded_tax_del .= ' ' . sprintf( __( '(Includes %s)', 'media_center' ), implode( ', ', $tax_del_array ) );
							}
							if ( ! empty( $tax_ins_array ) ) {
								$refunded_tax_ins .= ' ' . sprintf( __( '(Includes %s)', 'media_center' ), implode( ', ', $tax_ins_array ) );
							}
						}
			...
			if ( $has_refund ) { ?>
				<tr>
					<th scope="row"><?php _e( 'Refunded:', 'media_center' ); ?></th>
			...
			if ( '' != $order->customer_note ) { ?>
				<tr>
					<th scope="row"><?php _e( 'Note:', 'media_center' ); ?></th>
...
<section class="customer-details section">
	<h2><?php _e( 'Customer details', 'media_center' ); ?></h2>
	<table class="shop_table shop_table_responsive customer_details">
	<?php
		if ( $order->billing_email ) {
			echo '<tr><th>' . __( 'Email:', 'media_center' ) . '</th><td data-title="' . __( 'Email', 'media_center' ) . '">' . $order->billing_email . '</td></tr>';
		}
		if ( $order->billing_phone ) {
			echo '<tr><th>' . __( 'Telephone:', 'media_center' ) . '</th><td data-title="' . __( 'Telephone', 'media_center' ) . '">' . $order->billing_phone . '</td></tr>';
		}
	...
	<?php endif; ?>
			...
			<header class="title">
				<h3><?php _e( 'Billing Address', 'media_center' ); ?></h3>
			</header>
			<address>
				<?php
		...
		<div class="col-md-6 col-sm-6 col-xs-12 address">
			<header class="title">
				<h3><?php _e( 'Shipping Address', 'media_center' ); ?></h3>
			</header>
			<address>
				<?php
