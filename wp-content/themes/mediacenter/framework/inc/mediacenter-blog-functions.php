...
function media_center_blog_layout(){
	...
	return $media_center_theme_options['blog_layout'];
}
endif;
...
function media_center_blog_settings() {
	...
	$blog_layout = $media_center_theme_options['blog_layout'];
	...
	switch($blog_layout){
		...
		case 'without_sidebar';
			...
			$content_area_class	= ( $media_center_theme_options['full_width_density'] == 'wide' ) ? 'col-md-12' : 'col-md-9 center-block col-xs-12 col-sm-10 col-lg-9';
			$sidebar_class		= '';
			$has_sidebar		= false;
		break;
	}
...
if ( ! function_exists( 'media_center_post_layout' ) ) :
function media_center_post_layout(){
	global $media_center_theme_options;
	return $media_center_theme_options['blog_layout'];
}
endif;
...
//................................................................
/**
 * We set this pretty high because our "custom_excerpt" function 
...
function new_excerpt_length($length) {
	global $media_center_theme_options;
	return $media_center_theme_options['excerpt_length'];
}
endif;
...
if ( ! function_exists( 'media_center_post_content' ) ) :
	function media_center_post_content( $post_id = false, $content = false, $excerpt = false ) {
		global $post, $custom_query, $media_center_theme_options;

		$post_id = ($post_id) ? $post_id : $post->ID;
		$force_post_excerpts  = $media_center_theme_options['force_excerpt'] ; // Media Center Theme Options
		                                                   // 
		if ( ! is_single() && ( $force_post_excerpts || is_search() ) ) {
			...
			if( has_excerpt() ){
				...
				the_content( __( 'Continue reading <span class="meta-nav">&rarr;</span>', 'media_center' ) );
			}
			wp_link_pages( array( 'before' => '<div class="page-links">' . __( 'Pages:', 'media_center' ), 'after' => '</div>' ) );
		}
	}
endif;
...
if ( ! function_exists( 'media_center_post_meta' ) ):
	function media_center_post_meta(){
		...
		if ( $postFormat != 'quote' && $postFormat != 'link' ){
			echo '<ul class="meta">';
			echo '<li class="post-author">' . sprintf( __( 'Posted By : %s', 'media_center' ), media_center_post_author() ) . '</li>';
			if ( is_blog_layout_grid() || is_single() ){
				echo '<li class="date">'.get_the_date().'</li>';
			}
			echo '<li class="categories">'.get_the_category_list( _x( ', ', 'Used between list items, there is a space after the comma.', 'media_center' ) ). '</li>';
			if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) :
				echo '<li class="comments">';
					comments_popup_link( __( 'Leave a comment', 'media_center' ), __( '1 Comment', 'media_center' ), __( '% Comments', 'media_center' ) );
				echo '</li>';
			endif;
			edit_post_link( __( 'Edit', 'media_center' ), '<li class="edit-link">', '</li>' );
			echo '</ul>';
		}else{
...
function get_available_blog_layouts(){
	$layouts = array(
			'sidebar_right' => array(
				'label' => __( 'Sidebar Right' , 'media_center' ),
				'hasSidebar' => true,
				'isGrid' => false
			),
			'sidebar_left' => array(
				'label' => __( 'Sidebar Left' , 'media_center' ),
				'hasSidebar' => true,
				'isGrid' => false
			),
			'without_sidebar' => array(
				'label' => __( 'Without Sidebar' , 'media_center' ),
				'hasSidebar' => false,
				'isGrid' => false
			),
		);
...
function get_post_layout_details(){
	$post_layout = media_center_post_layout();
...
function media_center_comment( $comment, $args, $depth ) {
	...
	<li <?php comment_class('comment-item'); ?> id="comment-<?php comment_ID(); ?>">
		<p><?php _e( 'Pingback:', 'media_center' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( '(Edit)', 'media_center' ), '<span class="edit-link">', '</span>' ); ?></p>
	<?php
			break;
		default :
					...
					<header class="row no-margin">
						<div class="pull-left">
							...
							<?php if( $comment->user_id === $post->post_author ): ?>
								<span class="label label-default"><?php echo __( 'Post author', 'media_center' ); ?></span>
							<?php endif;?>
							<?php if ( '0' == $comment->comment_approved ) : ?>
								<span class="label label-danger"><?php _e( 'Awaiting moderation.', 'media_center' ); ?></span>
							<?php endif; ?>
							<span class="date"><?php echo get_comment_date(); ?></span>
						</div><!-- /.pull-left -->
						<div class="pull-right">
							<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Reply', 'media_center' ), 'after' => ' <span>&darr;</span>', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
						</div><!-- /.pull-right -->
					</header><!-- /.row -->
...
function media_center_content_nav( $html_id ) {
	...
	<nav class="navigation post-navigation inner-bottom-xs" role="navigation">
		<h1 class="sr-only screen-reader-text"><?php _e( 'Post navigation', 'media_center' ); ?></h1>
		<div class="nav-links clearfix">
			<?php
			if ( is_attachment() ) :
				previous_post_link( '%link', __( '%title', 'media_center' ) );
			else :
				echo '<div class="pull-left">';
				previous_post_link( '%link', __( '&larr; %title', 'media_center' ) );
				echo '</div>';
				echo '<div class="pull-right">';
				next_post_link( '%link', __( '%title &rarr;', 'media_center' ) );
				echo '</div>';
			endif;
			?>
		</div><!-- .nav-links -->
